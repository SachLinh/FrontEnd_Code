...
export default function CartItem({ value, index, deleteProductInCart }) {
  ...
  const increaseProductCount = () => {
    if (value.Count < value.CountWareHouse) {
    ...
    <tr className="">
      <td className="flex flex-row justify-start items-center w-[230px] sm:w-[400px]">
        ...
        <img
          src={value.Image}
          alt="product in cart"
          className="w-[70px] sm:w-[100px] mx-[10px]"
        />
        <div className="text-[13px] sm:text-[18px] text-[#389250]">
          {value.Name}
          <p className=" sm:hidden text-[#0f0404]">
      ...
      <td className="hidden md:block text-[15px] md:text-[20px] text-center w-[100px] lg:w-[200px]">
        {" "}
        {formatPrice(value.Price)}
      </td>
    </tr>
